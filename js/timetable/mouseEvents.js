...
function displayCourseInformation(courseObject, section) {
    var result = '<h2>' + courseObject.name + ': ' + courseObject.title + '</h2>';
    result += '<p>' + courseObject.description + '</p>';
    if (typeof courseObject.prereqString !== 'undefined' && courseObject.prereqString !== null) {
        result += '<p><strong>Prerequisite:</strong> ' + courseObject.prereqString + '</p>';
    }
    if (typeof courseObject.prep !== 'undefined' && courseObject.prep !== null) {
        result += '<p><strong>Recommended Preparation:</strong> ' + courseObject.prep + '</p>';
    }
    if (typeof courseObject.exclusions != 'undefined' && courseObject.exclusions !== null) {
        result += '<p><strong>Exclusions:</strong> ' + courseObject.exclusions + '</p>';
    }

    result += '<p><strong>Distribution Requirement Status:</strong> ' + courseObject.distribution + '</p>';
    result += '<p><strong>Breadth Requirement:</strong> ' + courseObject.breadth + '</p>';
